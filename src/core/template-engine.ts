/**
 * Template Engine for Universal Script Generator
    ...
    Handlebars.registerHelper('pythonImports', (libraries: string[]) => {
      ...
      const importLines = libraries.map(lib => {
        switch (lib) {
          ...
          case 'matplotlib': return 'import matplotlib.pyplot as plt';
          case 'requests': return 'import requests';
          case 'tkinter': return 'import tkinter as tk\nfrom tkinter import ttk, filedialog, messagebox';
          case 'sqlite3': return 'import sqlite3';
          case 'json': return 'import json';
          case 'os': return 'import os';
          case 'subprocess': return 'import subprocess';
          case 'logging': return 'import logging';
          default: return `import ${lib}`;
        }
      });
...
GREEN='\x1b[0;32m'    # Neon Green
CYAN='\x1b[0;36m'     # Electric Cyan  
PURPLE='\x1b[0;35m'   # Royal Purple
NAVY='\x1b[0;34m'     # Deep Navy
YELLOW='\x1b[1;33m'   # Bright Yellow
RED='\x1b[0;31m'      # Alert Red
NC='\x1b[0m'          # No Color`);
    });
  }
  ...
  private loadBuiltinTemplates(): void {
    ...
    this.registerTemplate({
      ...
      variables: [
        { name: 'description', type: 'string', required: true, description: 'Script description' },
        { name: 'imports', type: 'array', required: false, description: 'Required imports' },
        { name: 'mainLogic', type: 'string', required: true, description: 'Main script logic' }
      ],
      requirements: [],
    ...
    this.registerTemplate({
      ...
      variables: [
        ...
        { name: 'features', type: 'array', required: false, description: 'GUI features' }
      ],
    ...
    this.registerTemplate({
      ...
      variables: [
        { name: 'description', type: 'string', required: true, description: 'Processing description' },
        { name: 'fileTypes', type: 'array', required: false, description: 'Supported file types' },
        { name: 'processingLogic', type: 'string', required: true, description: 'File processing logic' }
      ],
    ...
    this.registerTemplate({
      ...
      variables: [
        { name: 'description', type: 'string', required: true, description: 'Script description' },
        { name: 'commands', type: 'array', required: false, description: 'System commands' },
        { name: 'mainLogic', type: 'string', required: true, description: 'Main script logic' }
      ],
      requirements: [],
      alsaniaCompliant: true
    ...
    this.registerTemplate({
      ...
      variables: [
        { name: 'description', type: 'string', required: true, description: 'Action description' },
        { name: 'actionName', type: 'string', required: true, description: 'Action name' },
        { name: 'fileLogic', type: 'string', required: true, description: 'File processing logic' }
      ],
      requirements: ['zenity'],
      alsaniaCompliant: true
    });
    ...
    this.registerTemplate({
      ...
      variables: [
        { name: 'description', type: 'string', required: true, description: 'Command description' },
        { name: 'originalCommand', type: 'string', required: true, description: 'Original terminal command' },
        { name: 'transformedLogic', type: 'string', required: true, description: 'Transformed command logic' }
      ],
    ...
    this.logger.info('Built-in templates loaded', { 
      ...
      alsaniaCompliant: Array.from(this.templates.values()).every(t => t.alsaniaCompliant)
    });
  }
  ...
  async processTemplate(
    ...
    try {
      ...
      const variables = this.prepareTemplateVariables(template, nlpAnalysis, overrides);
      ...
      const { errors, warnings, suggestions } = this.validateTemplateOutput(code, template);
  ...
  private selectTemplate(language: 'python' | 'bash', nlpAnalysis: NLPAnalysis): Template {
    ...
    switch (nlpAnalysis.intent.primary) {
      case 'gui_application':
        return candidates.find(t => t.category === 'gui') || candidates.find(t => t.category === 'basic')!;
      
      case 'nemo_integration':
        return candidates.find(t => t.category === 'nemo_integration') || candidates.find(t => t.category === 'basic')!;
      
      case 'kde_connect':
        return candidates.find(t => t.category === 'kde_connect') || candidates.find(t => t.category === 'basic')!;
      
      case 'file_processing':
        return candidates.find(t => t.category === 'file_processing') || candidates.find(t => t.category === 'basic')!;
      
      case 'web_automation':
        return candidates.find(t => t.category === 'web') || candidates.find(t => t.category === 'basic')!;
  ...
  private generateMainLogic(nlpAnalysis: NLPAnalysis): string {
    ...
    if (!logic) {
      ...
      logic += '    print("Script executed successfully!")\n';
      logic += '    # TODO: Implement custom logic based on your requirements\n';
    }

    return logic;
  }

  private getIntegrationVariables(nlpAnalysis: NLPAnalysis): Record<string, any> {
    const variables: Record<string, any> = {};
    ...
    switch (nlpAnalysis.intent.primary) {
      ...
      case 'kde_connect':
        ...
        variables.transformedLogic = '    ls -la && notify-send "Command executed"';
        break;
  ...
  private extractDependencies(template: Template, variables: Record<string, any>): string[] {
  ...
  private validateTemplateOutput(code: string, template: Template): {
    errors: string[];
    warnings: string[];
    suggestions: string[];
  } {
    ...
    if (!code.includes('try:') && !code.includes('catch') && template.language === 'python') {
  ...
  registerTemplate(template: Template): void {
    ...
    this.logger.debug('Template registered', { name: template.name, language: template.language });
  }
  ...
  async loadTemplatesFromDirectory(directory: string): Promise<void> {
    try {
      ...
      this.logger.warn('Could not load external templates', { directory, error });
    }
  }
  ...
  async healthCheck(): Promise<{ healthy: boolean; details: any }> {
    try {
      ...
      const alsaniaCompliantCount = Array.from(this.templates.values())
        .filter(t => t.alsaniaCompliant).length;
      ...
      return {
        ...
        details: {
          ...
          languages: Array.from(new Set(Array.from(this.templates.values()).map(t => t.language))),
...
class {{appName}}App:
    ...
    def setup_window(self):
        """Configure main window with Alsania styling"""
    ...
    def setup_theme(self):
        """Apply Alsania theme to ttk widgets"""
        style = ttk.Style()
...
class FileProcessor:
    """Alsania-powered file processing engine"""
    ...
    def process_files(self) -> bool:
        """Process files based on requirements"""
        try:
...
{{mainLogic}}
    ...
    def _process_directory(self, dir_path: Path) -> bool:
