/**
 * Integration Manager for Universal Script Generator
 * Alsania aligned - built by Sigma, powered by Echo
 */

import { Integration, IntegrationFile, GeneratorConfig, Logger, NLPAnalysis } from './types';
...
export class IntegrationManager {
  ...
  async generateIntegrationFiles(
    ...
    for (const integration of integrations) {
      ...
      try {
        this.logger.debug(`Generating ${integration.type} integration files`);
        ...
        this.logger.warn(`Failed to generate ${integration.type} integration`, error);
      }
    }
...
Mimetypes=application/x-shellscript;`;
    ...
    return [{
      type: 'nemo',
      filename: `${actionName.toLowerCase().replace(/\s+/g, '_')}.nemo_action`,
