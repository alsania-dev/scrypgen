/**
 * Enhanced NLP Parser for Universal Script Generator
 * Alsania aligned - built by Sigma, powered by Echo
 */

import { NLPAnalysis, Intent, Entity, ScriptRequirements, Logger } from './types';
...
export class EnhancedNLPParser {
  ...
  private initializeKeywords(): void {
    this.pythonKeywords = {
      'file_operations': ['read', 'write', 'create', 'delete', 'copy', 'move', 'rename', 'chmod'],
      'data_processing': ['parse', 'filter', 'sort', 'transform', 'analyze', 'convert', 'merge'],
      'web_scraping': ['scrape', 'crawl', 'download', 'fetch', 'request', 'api', 'json', 'xml'],
      'database': ['query', 'insert', 'update', 'delete', 'sql', 'sqlite', 'postgres', 'mysql'],
      'gui_desktop': ['tkinter', 'qt', 'gtk', 'window', 'dialog', 'button', 'menu', 'interface'],
      'automation': ['schedule', 'cron', 'automate', 'batch', 'monitor', 'watch', 'trigger'],
      'system': ['subprocess', 'process', 'command', 'execute', 'run', 'shell', 'os'],
      'network': ['http', 'https', 'ftp', 'ssh', 'socket', 'requests', 'urllib'],
      'math_science': ['calculate', 'compute', 'numpy', 'scipy', 'matplotlib', 'pandas', 'statistics'],
      'machine_learning': ['sklearn', 'tensorflow', 'pytorch', 'model', 'predict', 'train', 'classify'],
      'image_processing': ['pillow', 'opencv', 'image', 'photo', 'resize', 'crop', 'filter'],
      'audio_video': ['audio', 'video', 'ffmpeg', 'convert', 'encode', 'decode', 'stream'],
      'testing': ['unittest', 'pytest', 'test', 'assert', 'mock', 'fixture'],
      'logging': ['log', 'debug', 'error', 'warning', 'info', 'trace']
    };

    this.bashKeywords = {
      'file_operations': ['ls', 'cp', 'mv', 'rm', 'mkdir', 'rmdir', 'chmod', 'chown', 'find', 'locate'],
      'text_processing': ['grep', 'sed', 'awk', 'cut', 'sort', 'uniq', 'wc', 'tr', 'head', 'tail'],
      'system_admin': ['ps', 'kill', 'killall', 'service', 'systemctl', 'crontab', 'at', 'nohup'],
      'network': ['curl', 'wget', 'ping', 'ssh', 'scp', 'rsync', 'netstat', 'ss'],
      'archive': ['tar', 'zip', 'unzip', 'gzip', 'gunzip', 'compress', 'uncompress'],
      'monitoring': ['top', 'htop', 'ps', 'df', 'du', 'free', 'iostat', 'vmstat'],
      'package_management': ['apt', 'yum', 'dnf', 'pacman', 'pip', 'npm', 'snap'],
      'git_vcs': ['git', 'svn', 'commit', 'push', 'pull', 'clone', 'branch', 'merge'],
      'backup': ['backup', 'sync', 'mirror', 'archive', 'restore'],
      'environment': ['export', 'env', 'set', 'unset', 'source', 'alias'],
      'conditional': ['if', 'then', 'else', 'elif', 'fi', 'case', 'while', 'for', 'until'],
      'nemo_integration': ['nemo', 'file manager', 'context menu', 'right click', 'action'],
      'kde_connect': ['kde connect', 'phone', 'mobile', 'remote', 'notification', 'command']
    };
  }

  async analyzeDescription(description: string): Promise<NLPAnalysis> {
    try {
      this.logger.info('Analyzing natural language description', { description });
      ...
      const suggestedLanguage = this.determineBestLanguage(processedText, intent);
      
      // Extract requirements
      const requirements = this.extractRequirements(processedText, intent, entities);
  ...
  private extractIntent(text: string): Intent {
    ...
    if (actions.includes('nemo') || text.includes('file manager') || text.includes('context menu')) {
      primary = 'nemo_integration';
    } else if (actions.includes('kde') || text.includes('phone') || text.includes('remote command')) {
      primary = 'kde_connect';
    } else if (actions.some(action => ['scrape', 'download', 'api', 'http'].includes(action))) {
      primary = 'web_automation';
    } else if (actions.some(action => ['backup', 'sync', 'archive'].includes(action))) {
      primary = 'system_administration';
    } else if (actions.some(action => ['gui', 'window', 'interface'].includes(action))) {
      primary = 'gui_application';
    } else if (actions.some(action => ['read', 'write', 'process', 'parse'].includes(action))) {
  ...
  private extractEntities(text: string): Entity[] {
    ...
    // File patterns
    const filePattern = /\b[\w-]+\.(txt|csv|json|xml|pdf|doc|xls|py|sh|js|html|css|log)\b/g;
    let match;
  ...
  private assessComplexity(text: string, intent: Intent, entities: Entity[]): 'simple' | 'medium' | 'complex' {
    let complexityScore = 0;

    // Base complexity indicators
    const simpleIndicators = ['hello', 'basic', 'simple', 'print', 'echo'];
    const mediumIndicators = ['function', 'class', 'loop', 'condition', 'if', 'for', 'while'];
    const complexIndicators = [
      'algorithm', 'optimization', 'concurrent', 'threading', 'async',
      'machine learning', 'ai', 'neural', 'deep learning',
      'database', 'sql', 'api', 'microservice',
    ...
    if (intent.primary === 'nemo_integration' || intent.primary === 'kde_connect') {
      complexityScore += 1;
    }
  ...
  private determineBestLanguage(text: string, intent: Intent): 'python' | 'bash' {
    let pythonScore = 0;
    let bashScore = 0;
    ...
    switch (intent.primary) {
      ...
      case 'file_processing':
        ...
        if (intent.actions.includes('parse') || intent.actions.includes('analyze')) {
          pythonScore += 1;
        } else {
          bashScore += 1;
        }
    ...
    if (text.includes('.sh') || text.includes('bash') || text.includes('shell')) bashScore += 2;
  ...
  private getCategoryWeight(category: string, language: 'python' | 'bash'): number {
  ...
  private extractRequirements(text: string, intent: Intent, entities: Entity[]): ScriptRequirements {
    ...
    entities.forEach(entity => {
      if (entity.label === 'FILE') {
        if (text.includes('read') || text.includes('input') || text.includes('load')) {
          requirements.inputFiles.push(entity.text);
        }
        if (text.includes('write') || text.includes('output') || text.includes('save')) {
    ...
    requirements.networkAccess = intent.actions.some(action => 
      ['download', 'upload', 'request', 'fetch', 'scrape', 'ping'].includes(action)
    ) || entities.some(entity => entity.label === 'URL');

    requirements.fileSystemAccess = intent.actions.some(action =>
      ['read', 'write', 'create', 'delete', 'copy', 'move', 'find'].includes(action)
    ) || entities.some(entity => ['FILE', 'PATH'].includes(entity.label));

    requirements.guiRequired = intent.primary === 'gui_application' || 
      intent.actions.some(action => ['show', 'display', 'window'].includes(action));

    requirements.webRequired = intent.primary === 'web_automation' || requirements.networkAccess;

    requirements.databaseRequired = intent.actions.some(action =>
      ['query', 'insert', 'update', 'delete'].includes(action)
    ) || text.includes('database') || text.includes('sql');

    requirements.errorHandling = intent.secondary.includes('error_handling') ||
      text.includes('error') || text.includes('exception') || text.includes('handle');
